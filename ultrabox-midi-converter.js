...
function generateMidi(data) {
    ...
    channels.forEach((channel, channelIndex) => {
        // pitch（メロディ/コード）と drum（ドラム）チャンネルを処理対象とする
        if (channel.type !== "pitch" && channel.type !== "drum") return;
        
        const track = new MidiWriter.Track();
        track.setTempo(bpm);
        
        let midiChannel = channelIndex + 1; // MIDIチャンネルは1から始まる
        if (channel.type === "drum") {
            midiChannel = 10; // ドラムチャンネルはMIDIチャンネル10
        }
        track.setChannel(midiChannel); // トラックのMIDIチャンネルを設定
        ...
        // 音色設定 (General MIDI Program Change)
        if (channel.type === "pitch") {
            // 楽器インデックスに基づいてGM音色を設定
            // 複雑なUltraBoxの音色はMIDIで再現できないため、GM互換の番号を使用
        ...
        for (let barIndex = 0; barIndex < loopBars; barIndex++) {
            ...
            patternData.notes.forEach(note => {
                ...
                // ベロシティ (UltraBoxの音量からMIDIのベロシティへ変換)
        ...
        // イベントを時間順（ティック順）にソート (最も重要)
        trackEvents.sort((a, b) => a.tick - b.tick);

        // デルタタイムを計算しながらMIDIトラックに追加
        let lastTick = 0;
        trackEvents.forEach(event => {
            const deltaTime = event.tick - lastTick;
            
            if (deltaTime < 0) return; // 理論上ありえないが、安全のため

            if (event.type === 'on') {
                track.addEvent(new MidiWriter.NoteOnEvent({
                    pitch: [event.pitch], 
                    velocity: event.velocity, 
                    duration: 'T' + deltaTime // ティック単位のデルタタイム
                }));
...
/**
 * ArrayBufferをMIDIファイルとしてダウンロードさせる。
 * @param {ArrayBuffer} arrayBuffer - MIDIバイナリデータ。
...
// ===================================================================================
// 4. HTML DOM操作とメインエントリポイント (ブラウザ環境依存)
// ===================================================================================

document.addEventListener('DOMContentLoaded', () => {
    // この関数はブラウザのDOMがロードされた後に実行されるため、
    // HTMLの要素（ボタン、入力欄など）がこのスクリプトと同じページに存在する必要があります。
    // GitHubに公開する際は、このスクリプトを読み込むための index.html も提供してください。
    ...
    if (fileInput && convertButton) {
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (ultraBoxData.format === "UltraBox" || ultraBoxData.format === "BeepBox") {
                        convertButton.disabled = false;
                        showMessage(`「${file.name}」を読み込みました。変換ボタンを押してください。`, 'success');
                    } else {
                        throw new Error("このファイルはUltraBox/BeepBoxのJSON形式ではないようです。");
                    }
                } catch (error) {
                    showMessage("ファイルの解析に失敗しました。JSONファイルが壊れていないか確認してください。", 'error');
                    console.error(error);
                    ultraBoxData = null;
                }
            };
        ...
        // HTML要素が見つからない場合（例えばコンソールでの実行時など）
        console.error("HTML要素 (jsonFile, convertButton) が見つかりませんでした。HTMLファイルを確認してください。");
    }
});

// GitHubで公開する場合、以下の index.html も併せて提供することで、
// ユーザーがすぐに機能を利用できるようになります。
